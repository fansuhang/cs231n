...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    exponential = exponential.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    ...
    numerator = numerator.to(device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
